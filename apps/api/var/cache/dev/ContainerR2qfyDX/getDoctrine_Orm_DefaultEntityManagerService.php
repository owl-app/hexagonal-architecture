 ...
 */
class getDoctrine_Orm_DefaultEntityManagerService extends Owl_Apps_Api_ApiKernelDevDebugContainer
{
    /**
    ...
    {
        if (true === $lazyLoad) {
            return $container->services['doctrine.orm.default_entity_manager'] = $container->createProxy('EntityManagerGhost4a7adfd', fn () => \EntityManagerGhost4a7adfd::createLazyGhost(fn ($proxy) => self::do($container, $proxy)));
        }
        ...
        include_once \dirname(__DIR__, 6).'/vendor/doctrine/orm/lib/Doctrine/ORM/Mapping/Driver/XmlDriver.php';
        include_once \dirname(__DIR__, 6).'/vendor/doctrine/orm/lib/Doctrine/ORM/Mapping/Driver/SimplifiedXmlDriver.php';
        ...
        $c = new \Doctrine\ORM\Mapping\Driver\SimplifiedXmlDriver([(\dirname(__DIR__, 6).'/src/Article/Infrastructure/Persistence/Doctrine') => 'Owl\\Article\\Domain\\Model']);
        $c->setGlobalBasename('mapping');

        $b->addDriver($c, 'Owl\\Article\\Domain\\Model');

        $a->setEntityNamespaces(['Article' => 'Owl\\Article\\Domain\\Model']);
        ...
        $a->setRepositoryFactory(new \Doctrine\Bundle\DoctrineBundle\Repository\ContainerRepositoryFactory(new \Symfony\Component\DependencyInjection\Argument\ServiceLocator($container->getService, [
            'Owl\\Article\\Infrastructure\\Persistence\\ArticleRepository' => ['privates', 'Owl\\Article\\Infrastructure\\Persistence\\ArticleRepository', 'getArticleRepositoryService', true],
        ], [
            'Owl\\Article\\Infrastructure\\Persistence\\ArticleRepository' => '?',
        ])));
